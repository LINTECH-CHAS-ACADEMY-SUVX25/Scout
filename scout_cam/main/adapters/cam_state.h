...
/**
 * @brief Inits the camera with retry + backoff. Calls esp_restart() on repeated failure —
...
/**
 * @brief Called from stream_run when streaming is false.
 *        Checks for WiFi reconnect and sets cam_status.streaming = true if reconnected.
 */
void cam_state_try_resume(void);

/**
 * @brief Applies a control command received from scout_screen on CTRL_PORT.
