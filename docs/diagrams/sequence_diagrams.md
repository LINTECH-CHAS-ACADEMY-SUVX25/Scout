...
sequenceDiagram
    ...
    S->>S: lvgl_port_init()
    S->>S: scout_ui_init()
    Note right of S: Intro loading bar
    S->>S: wifi_ap_start()
    ...
    C->>C: cam_state_init()
    C->>C: cam_state_camera_start()
    Note right of C: 3 attempts, 0/500/2000 ms backoff
    C->>C: motor_init()
    C->>C: bme280_init()
    C->>C: stream_init()
    Note right of C: udp_open(VID_PORT tx)\nudp_open(CMD_PORT + DIAG_PORT + CTRL_PORT rx)
...
to the render task via ping-pong PSRAM buffers.
...
via `rc_rx` and forwards the x/y coordinates to the motor task via a FreeRTOS queue.

```mermaid
sequenceDiagram
    participant UI  as LVGL / scout_ui (Screen)
    participant RR  as render_run (Screen, core 1)
    participant CC  as rc_tx
    participant NET as UDP / WiFi
    participant SR  as stream_run (Cam)
    participant MQ  as motor_queue
    ...
    RR->>CC: rc_tx_send_throttled(jx, jy)
