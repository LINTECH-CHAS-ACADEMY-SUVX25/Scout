...
/** @brief Opens a UDP socket. bind_port 0 = send-only. Returns fd or -1 on failure. */
...
/** @brief Blocking receive. src may be NULL if the source address is not needed. */
int  udp_rx(int sock, void *buf, size_t len, struct sockaddr_in *src);

/** @brief Non-blocking receive. Returns 0 if nothing is waiting, -1 on error. */
